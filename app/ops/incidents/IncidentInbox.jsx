...
export default function IncidentInbox() {
  ...
  return (
    <div className="command-workspace min-h-full p-6 space-y-4" style={{ background: 'var(--base)', color: 'var(--text)' }}>
      <PageHeader icon={<ShieldAlert size={20} />} title="Incident Inbox" subtitle={snapshot.telemetryAvailable === false ? 'Saved incidents for this installation.' : 'Platform health and relayed errors, deduplicated into an operator-owned response queue.'}
      ...
      {snapshot.warning && <div role="status" className="rounded-lg px-4 py-3 text-sm" style={{ color: 'var(--text-muted)', background: 'var(--surface)', border: '1px solid var(--border)' }}>{snapshot.warning}</div>}
