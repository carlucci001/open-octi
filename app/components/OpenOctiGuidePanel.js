...
export default function OpenOctiGuidePanel({ compact = false, initialPrompt = '' }) {
  const [enabled, setEnabled] = useState(false); const [messages, setMessages] = useState([{ role: 'assistant', content: greeting }]); const [input, setInput] = useState(initialPrompt); const [busy, setBusy] = useState(false)
  ...
  const send = async event => {
    ...
    try {
      const response = await fetch('/api/openocti/guide', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ messages: next }) })
