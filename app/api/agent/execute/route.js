...
async function authenticateAgentRequest(request) {
  const allowed = [
  ...
  if (allowed.some(secret => machineSecretMatches(header, secret))) return { kind: 'machine' }
  ...
  if (user) return { kind: 'session', userId: user.id || null, role: user.role || null }
  ...
  if (!isOpenOcti() && process.env.NODE_ENV !== 'production' && !allowed.length) return { kind: 'development' }
  return null
}
