...
async function computeStatus() {
  const [local, publicCrm, crmService, cloudflared, workingDirectory, openclaw, externals] = await Promise.all([
    ...
    withTimeout(serviceStatus('farrington-crm.service', 'OpenOcti CRM'), 3000, { ok: false, error: 'timeout' }),
...
export async function GET(request) {
  ...
  if (isOpenOcti()) return NextResponse.json(await openOctiSystemStatus())
  const now = Date.now()
  if (CACHE) {
