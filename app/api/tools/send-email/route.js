...
import { hasOutboundEmailTransport, sendOutboundEmail } from '@/lib/outbound-email'
...
export async function POST(request) {
  ...
  try {
    const { to, subject, body, from, replyTo, html: htmlOverride, attachments: attachmentSpecs, agent: agentSpec, brand: brandKey, leadId } = await request.json()
    if (leadId) assertLeadSignalChannelAllowed({ leadId, channel: 'email' })
    if (!to || !to.includes('@')) {
    ...
    if (!hasOutboundEmailTransport()) {
      return NextResponse.json({ ok: false, error: 'Resend is not configured' }, { status: 400, headers: cors })
    }
    ...
    await sendOutboundEmail(sendPayload)
