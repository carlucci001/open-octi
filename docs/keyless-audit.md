...
| Vercel deployments | `vercel` | Deploy requests reached an unconfigured provider route. | Vercel API paths fail closed and Admin provides setup and connection testing. |
...
| Email sends and templates | `resend` or `nylas` | Sends could expose raw missing-key errors or use owner-specific fallback addresses. | Sender/reply-to values are environment driven; email capabilities and provider tests are listed in Admin. |
...
| Leads Lab paid sources | `apify` | Paid actor runs could be attempted without a token or actor/task. | The manifest requires an actor or task plus `APIFY_API_TOKEN`; Admin displays both groups. |
