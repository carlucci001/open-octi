import { editionFor } from './edition'
...
export const EXTERNAL_CAPABILITIES = Object.freeze([
  ...
  definition('getfound3', 'GetFound3', [['GETFOUND3_API_URL'], ['GETFOUND3_API_KEY']], ['/api/getfound3']),
...
export function buildFeatureManifest(env = process.env) {
  const capabilities = EXTERNAL_CAPABILITIES.map(item => capabilityStatus(item.id, env))
  return {
