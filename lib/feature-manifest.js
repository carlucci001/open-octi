...
export function capabilityStatus(id, env = process.env) {
  ...
  // The private site-note route already uses the configured company website by default.
  // OpenOcti installations must supply their own endpoint.
  const usesSiteNoteDefault = id === 'site-note' && editionFor(env) === 'commandcenter' && !env?.SITE_NOTE_ENDPOINT
  const requirementGroups = id === 'daily' && isOpenOcti(env) ? [['DAILY_API_KEY']] : capability.requirementGroups
  const missing = requirementGroups
    .filter(group => !group.some(name => hasValue(env, name) || (usesSiteNoteDefault && name === 'SITE_NOTE_ENDPOINT')))
    .map(group => group[0])
  return {
    id: capability.id,
    label: capability.label,
    needs: requirementGroups.flat(),
    requirementGroups: requirementGroups.map(group => [...group]),
    missing,
    ...
    settings: settingsLinksForNeeds(requirementGroups.flat()),
  }
}

const OPENOCTI_APP_MANAGED_CAPABILITIES = new Set(['models', 'anthropic', 'openai', 'gemini', 'openrouter', 'orcarouter', 'elevenlabs', 'daily', 'stripe', 'stripe-client'])
