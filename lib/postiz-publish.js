...
export async function publishPostizPost({
  ...
  if (normalizedMediaUrl) {
    ...
    if (!upload.ok) {
      if (isOpenOcti()) throw new PostizPublishError('The image upload failed. No post was scheduled. Check image access and Postiz storage, then retry.', { status: 502, stage: 'media-upload', code: 'media_upload_failed' })
  ...
  if (isOpenOcti() && normalizedMediaUrl && !image.length) throw new PostizPublishError('Postiz did not confirm the image upload. No post was scheduled.', { status: 502, stage: 'media-upload', code: 'media_upload_invalid' })
