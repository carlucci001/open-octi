import { readData } from './dataStore'
import { getPostizConfig, POSTIZ_DEFAULT_TENANT } from './postiz-config'
import { isOpenOcti } from './edition'

export class PostizPublishError extends Error {
  constructor(message, { status = 502, stage = 'postiz-publish', code = '', detail = null, contentType = '' } = {}) {
    super(message)
    this.name = 'PostizPublishError'
    this.status = status
    this.stage = stage
    this.code = code
    this.detail = detail
    this.contentType = contentType
  }
}

export function getPostizPublishConfig(env = process.env) {
  return getPostizConfig(env)
}

export function getPostizPublishReadiness(env = process.env) {
  const config = getPostizPublishConfig(env)
  if (!config) {
    return {
      configured: false,
      connected: false,
      reason: 'Postiz env not configured (POSTIZ_API_URL / POSTIZ_API_KEY)',
    }
  }
  if (config.error) {
    return { configured: false, connected: false, reason: config.error }
  }
  return {
    configured: true,
    connected: true,
    publicUrl: config.publicUrl,
  }
}

export function normalizePostizMediaUrl(mediaUrl, requestUrl, env = process.env) {
  const raw = String(mediaUrl || '').trim()
  if (!raw) return ''
  try {
    const url = requestUrl ? new URL(raw, requestUrl) : new URL(raw)
    // Internal CRM callers still need a public URL for Postiz to fetch an image.
    // Keep the private installation's fallback out of independent OpenOcti installs.
    if (!isOpenOcti(env) && /^(localhost|127\.0\.0\.1|\[::1\])$/.test(url.hostname)
      && /^\/(api\/media\/file|media)\//.test(url.pathname)) {
      const publicBase = env.CRM_PUBLIC_URL || env.PUBLIC_APP_URL || 'https://crm.company.example.com'
      return new URL(`${url.pathname}${url.search}`, new URL(publicBase).origin).toString()
    }
    return url.toString()
  } catch {
    return raw
  }
}

function summarizePostizBody(text, contentType) {
  const raw = String(text || '')
  if (!raw) return null
  if (/html/i.test(contentType || '') || /^\s*<!doctype\s+html/i.test(raw) || /^\s*<html[\s>]/i.test(raw)) {
    return 'Postiz returned HTML instead of JSON. Check POSTIZ_API_URL; it should be the Public API URL, usually ending in /api/public/v1 on the self-hosted instance.'
  }
  return raw.slice(0, 600)
}

async function postizFetch(config, path, init = {}) {
  const url = `${config.base}${path}`
  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), init.timeoutMs || 15000)
  try {
    const response = await fetch(url, {
      ...init,
      signal: controller.signal,
      headers: {
        Authorization: config.key,
        'User-Agent': 'fcc-campaign-studio/1',
        ...(init.body ? { 'Content-Type': 'application/json' } : {}),
        ...(init.headers || {}),
      },
      cache: 'no-store',
    })
    const text = await response.text()
    const contentType = response.headers.get('content-type') || ''
    let body = null
    try { body = JSON.parse(text) } catch {}
    return {
      ok: response.ok,
      status: response.status,
      contentType,
      body,
      detail: body || summarizePostizBody(text, contentType),
    }
  } catch (error) {
    return {
      ok: false,
      status: 0,
      contentType: '',
      body: null,
      detail: String(error?.message || error),
    }
  } finally {
    clearTimeout(timeout)
  }
}

function resolveConfig(config) {
  const resolved = config || getPostizPublishConfig()
  if (!resolved) {
    throw new PostizPublishError('Postiz env not configured (POSTIZ_API_URL / POSTIZ_API_KEY)', {
      status: 503,
      stage: 'configuration',
    })
  }
  if (resolved.error) {
    throw new PostizPublishError(resolved.error, { status: 503, stage: 'configuration' })
  }
  return resolved
}

function enforceTenantChannels(channels, tenantId, tenantAssignments, brandId = '') {
  const raw = tenantAssignments || readData('postiz-channel-tenants.json') || {}
  const map = raw.map && typeof raw.map === 'object' ? raw.map : {}
  const defaultTenantId = raw.defaultTenantId || POSTIZ_DEFAULT_TENANT
  const wrongTenant = channels.filter(id => (map[id] || defaultTenantId) !== tenantId)
  if (wrongTenant.length) {
    throw new PostizPublishError(`Channel(s) not assigned to tenant ${tenantId}: ${wrongTenant.join(', ')}`, {
      status: 403,
      stage: 'tenant-validation',
      code: 'channel_tenant_mismatch',
    })
  }
  const cleanBrandId = String(brandId || '').trim()
  if (cleanBrandId) {
    const brandMap = raw.brandMap && typeof raw.brandMap === 'object' ? raw.brandMap : {}
    const defaultBrandId = raw.defaultBrandId || POSTIZ_DEFAULT_TENANT
    const wrongBrand = channels.filter(id => (brandMap[id] || defaultBrandId) !== cleanBrandId)
    if (wrongBrand.length) {
      throw new PostizPublishError(`Channel(s) not assigned to brand ${cleanBrandId}: ${wrongBrand.join(', ')}`, {
        status: 403,
        stage: 'brand-validation',
        code: 'channel_brand_mismatch',
      })
    }
  }
}

async function resolveIntegrationSettings(config, channelIds) {
  const response = await postizFetch(config, '/integrations')
  if (!response.ok || !Array.isArray(response.body)) {
    throw new PostizPublishError('Postiz channels could not be checked. Recheck the connection before scheduling.', { stage: 'channel-check', code: 'channel_check_failed' })
  }
  return new Map(channelIds.map(id => {
    const channel = response.body.find(item => item?.id === id)
    if (!channel || channel.disabled || typeof channel.identifier !== 'string' || !/^[a-z0-9-]+$/.test(channel.identifier)) {
      throw new PostizPublishError('A selected Postiz channel is unavailable or has no platform identifier. Reconnect it and select it again before scheduling.', { status: 400, stage: 'channel-check', code: 'channel_unavailable' })
    }
    return [id, { __type: channel.identifier }]
  }))
}

export async function publishPostizPost({
  content,
  mediaUrl = '',
  channels = [],
  publishAt = '',
  tenantId = POSTIZ_DEFAULT_TENANT,
  brandId = '',
  requestUrl = '',
  config,
  tenantAssignments,
} = {}) {
  const resolvedConfig = resolveConfig(config)
  const cleanContent = String(content || '').trim()
  const cleanChannels = Array.isArray(channels) ? channels.filter(Boolean) : []
  const cleanTenantId = String(tenantId || POSTIZ_DEFAULT_TENANT).trim()

  if (!cleanContent) throw new PostizPublishError('content is required', { status: 400, stage: 'validation' })
  if (!cleanChannels.length) throw new PostizPublishError('At least one channel is required', { status: 400, stage: 'validation' })
  enforceTenantChannels(cleanChannels, cleanTenantId, tenantAssignments, brandId)
  const integrationSettings = isOpenOcti() ? await resolveIntegrationSettings(resolvedConfig, cleanChannels) : null

  let when = publishAt ? new Date(publishAt) : new Date(Date.now() + 60000)
  if (Number.isNaN(when.getTime()) || when.getTime() < Date.now() + 30000) {
    when = new Date(Date.now() + 60000)
  }
  const scheduledFor = when.toISOString()
  const normalizedMediaUrl = normalizePostizMediaUrl(mediaUrl, requestUrl)
  let media = null

  if (normalizedMediaUrl) {
    const upload = await postizFetch(resolvedConfig, '/upload-from-url', {
      method: 'POST',
      body: JSON.stringify({ url: normalizedMediaUrl }),
      timeoutMs: 12000,
    })
    if (!upload.ok) {
      throw new PostizPublishError('The image upload failed. No post was scheduled. Check image access and Postiz storage, then retry.', { status: 502, stage: 'media-upload', code: 'media_upload_failed' })
    } else {
      media = upload.body || null
    }
  }

  const image = media?.id && media?.path ? [{ id: media.id, path: media.path }] : []
  if (normalizedMediaUrl && !image.length) throw new PostizPublishError('Postiz did not confirm the image upload. No post was scheduled.', { status: 502, stage: 'media-upload', code: 'media_upload_invalid' })
  const response = await postizFetch(resolvedConfig, '/posts', {
    method: 'POST',
    body: JSON.stringify({
      type: 'schedule',
      shortLink: false,
      date: scheduledFor,
      tags: [],
      posts: cleanChannels.map(integrationId => ({
        integration: { id: integrationId },
        value: [{ content: cleanContent, image }],
        settings: integrationSettings?.get(integrationId) || {},
      })),
    }),
  })

  if (!response.ok) {
    throw new PostizPublishError(`Postiz posts ${response.status}`, {
      status: 502,
      stage: 'create-post',
      detail: response.detail,
      contentType: response.contentType,
    })
  }

  const posts = Array.isArray(response.body) ? response.body : (response.body ? [response.body] : [])
  if (isOpenOcti() && (!posts.length || posts.some(post => !post?.postId && !post?.id))) throw new PostizPublishError('Postiz did not return a post reference for every result. Check its queue before retrying.', { status: 502, stage: 'create-post', code: 'invalid_post_response' })
  return {
    ok: true,
    postId: posts[0]?.postId || posts[0]?.id || '',
    group: posts[0]?.group || '',
    scheduleUrl: `${resolvedConfig.publicUrl}/launches`,
    scheduledFor,
    count: posts.length,
    integrationIds: posts.map(post => post?.integration?.id || post?.integration).filter(Boolean),
  }
}
