...
export function hardenOpenOctiToolPolicies(cfg) {
  cfg.tools = { ...(cfg.tools || {}), profile: 'minimal' }
  ...
  for (const agent of cfg.agents?.list || []) {
    ...
    agent.tools = { ...tools, profile: 'full', allow }
...
export async function patchOpenclawAgents(mutations, { reason = 'agent-manager' } = {}) {
  ...
  if (!SSH_TARGET) {
    ...
    if (isOpenOcti()) hardenOpenOctiToolPolicies(cfg)
...
if HARDEN:
    cfg["tools"] = dict(cfg.get("tools") or {})
    cfg["tools"]["profile"] = "minimal"
    cfg["tools"].pop("allow", None)
    cfg["tools"].pop("alsoAllow", None)
    for agent in lst:
        tools = dict(agent.get("tools") or {})
        configured = tools.get("allow") if isinstance(tools.get("allow"), list) else tools.get("alsoAllow", [])
        allow = []
        for value in configured:
            name = str(value or "").strip()
            if name.startswith("fcc_") and name != "fcc_call" and name not in allow:
                allow.append(name)
        if "fcc_list_tools" not in allow:
            allow.append("fcc_list_tools")
        tools["profile"] = "full"
        tools["allow"] = allow
...
export async function restoreOpenclawBackup(backupPath) {
  ...
  if (!SSH_TARGET) {
    if (isOpenOcti()) {
      const cfg = JSON.parse(await fs.readFile(backupPath, 'utf8'))
      hardenOpenOctiToolPolicies(cfg)
      await fs.writeFile(`${REMOTE_CONFIG}.tmp`, JSON.stringify(cfg, null, 2), 'utf8')
      await fs.rename(`${REMOTE_CONFIG}.tmp`, REMOTE_CONFIG)
    } else {
...
SOURCE = ${JSON.stringify(backupPath)}
CONFIG = ${JSON.stringify(REMOTE_CONFIG)}
HARDEN = ${isOpenOcti() ? 'True' : 'False'}
if not HARDEN:
