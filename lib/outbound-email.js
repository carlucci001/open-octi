...
export async function sendOutboundEmail(payload, options = {}) {
  ...
  const defaultFrom = process.env.RESEND_FROM || 'Your organization <redacted@example.invalid>'
  ...
  let result = requestOptions ? await client.emails.send(message, requestOptions) : await client.emails.send(message)
  const errorMessage = result?.error?.message || ''
  if (options.allowFallback !== false && result?.error && fallbackFrom !== message.from && /domain|verify|authorization|permission|sender/i.test(errorMessage)) {
    result = requestOptions
      ? await client.emails.send({ ...message, from: fallbackFrom }, { idempotencyKey: `${options.idempotencyKey}/fallback` })
      : await client.emails.send({ ...message, from: fallbackFrom })
  }
