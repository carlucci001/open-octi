...
function derivePublicStreamUrl(requestUrl, env = process.env) {
  ...
  const url = new URL(requestUrl || env.NEXT_PUBLIC_APP_URL || 'https://crm.company.example.com')
  if (/^(localhost|127\.0\.0\.1)(:\d+)?$/.test(url.host)) {
    const publicUrl = new URL(env.NEXT_PUBLIC_APP_URL || 'https://crm.company.example.com')
...
function bridgeStatus(env = process.env) {
  ...
  const publicBaseUrl = clean(env.TWILIO_AGENT_VOICE_BASE_URL || env.NEXT_PUBLIC_APP_URL || 'https://crm.company.example.com', '', 500)
  return {
    ok: true,
...
${parameterXml}
  ...
  <Say voice="alice">The OpenOcti agent bridge has ended. Goodbye.</Say>
</Response>`
}
