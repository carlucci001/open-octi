...
export function findExistingLeadMatch(candidate = {}, leads = [], options = {}) {
  ...
  for (const lead of leads || []) {
    ...
    if (candidate.signal?.sourceId && candidate.signal.sourceId === lead.signal?.sourceId) {
      const ids = [lead.signal.externalId, ...(lead.signal.externalIds || [])]
      if ([candidate.signal.externalId, ...(candidate.signal.externalIds || [])].some(id => id && ids.includes(id))) {
        return { lead, reason: 'official source reference already imported' }
      }
    }
    ...
    if (business && business === existingBusiness) {
      if (options.businessOnly || !person || !existingPerson || person === existingPerson) {
