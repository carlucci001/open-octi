...
export function publicOrigin(request) {
  ...
  if (!request) return isOpenOcti() ? 'http://localhost:3000' : 'https://openocti.local'
  const proto = request?.headers?.get('x-forwarded-proto') || 'https'
  const host = request?.headers?.get('x-forwarded-host') || request?.headers?.get('host') || 'openocti.local'
  const origin = `${proto}://${host}`.replace(/\/$/, '')
  return /portal\.farringtondevelopment\.com/i.test(origin) ? 'https://openocti.local' : origin
}
