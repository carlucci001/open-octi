...
export function signingConfiguration(env = process.env) {
  const missing = []
  // Command Center already has a public CRM origin fallback in publicOrigin().
  // OpenOcti installations must explicitly supply their own signing origin.
  if (isOpenOcti(env) && !String(env.SIGNING_PUBLIC_URL || '').trim()) missing.push('SIGNING_PUBLIC_URL')
  ...
  return {
    ...
    message: missing.length === 0
      ...
      : `Not configured — add ${missing.join(' and ')} to enable e-signature.`,
  }
}
