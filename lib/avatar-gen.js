...
import { octiAvatarFor } from './brand-assets'
import { defaultAgentAvatar } from './openocti-agent-avatars'
...
export function getAvatarMeta(id, localAvatar = null) {
  const brandAvatar = octiAvatarFor(id)
  if (brandAvatar) return { url: brandAvatar, provider: 'openocti-brand' }
  const meta = readMeta()
  const fallback = typeof localAvatar === 'string' ? { url: localAvatar, provider: 'custom' } : localAvatar?.url ? localAvatar : null
  const item = meta[id] || fallback
  if (!item) return defaultAgentAvatar(id)
  try {
