...
// Incremental RFC-4180 parser for large vendor exports. The caller controls
// chunk size, so a 100k+ row CSV never needs to be materialized as one string.
export async function * parseCsvStream(chunks) {
  let row = []; let value = ''; let quoted = false; let pendingQuote = false; let skipLf = false
  const finishRow = () => {
    row.push(value); value = ''
    const output = row; row = []
    return output.some(cell => cell !== '') ? output : null
  }
  for await (const input of chunks || []) {
    ...
    for (let index = 0; index < chunk.length; index += 1) {
      ...
      if (char === '"') quoted = true
        ...
        const output = finishRow(); if (output) yield output
      } else value += char
    }
  }
  if (pendingQuote) quoted = false
  if (value !== '' || row.length) { const output = finishRow(); if (output) yield output }
}
