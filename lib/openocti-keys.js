...
export const OPENOCTI_MODEL_PROVIDERS = Object.freeze([
  { id: 'daily', name: 'Daily video conferences', vaultNames: ['Daily', 'Daily.co'], envKeys: ['DAILY_API_KEY'], testUrl: 'https://api.daily.co/v1/rooms?limit=1' },
  ...
  { id: 'orcarouter', name: 'OrcaRouter', vaultNames: ['OrcaRouter', 'Orca Router'], envKeys: ['ORCAROUTER_API_KEY'], testUrl: 'https://api.orcarouter.ai/v1/models', openclawId: 'orcarouter', api: 'openai-completions', baseUrl: 'https://api.orcarouter.ai/v1', model: 'orcarouter/free', modelName: 'OrcaRouter Free Router', contextWindow: 65536, maxTokens: 8192, input: ['text'] },
])
...
function readStore(env, { strict = false } = {}) {
  try {
    const parsed = JSON.parse(fs.readFileSync(openOctiKeyStorePath(env), 'utf8'))
    if (parsed?.version !== STORE_VERSION || parsed?.algorithm !== ALGORITHM || !parsed.keys || typeof parsed.keys !== 'object' || Array.isArray(parsed.keys)) {
      if (strict) throw new Error('Invalid encrypted key store')
      return emptyStore()
    }
    return parsed
  } catch (error) {
    if (strict && error.code !== 'ENOENT') throw new Error('The encrypted key store could not be read. No credentials were changed.')
    return emptyStore()
  }
}
...
// Installation integrations share the same encrypted store as model keys.
// They are deliberately separate from the model-provider list and OpenClaw sync.
export function readInstallationIntegration(id, env = process.env) {
  if (!isOpenOcti(env) || id !== 'stripe') return null
  const record = readStore(env, { strict: true }).keys?.[`integration:${id}`]
  if (!record) return null
  try { return JSON.parse(decryptKey(record, env)) } catch {
    throw new Error('Saved integration configuration could not be decrypted. Check this installation\'s session secret.')
  }
}

export function writeInstallationIntegration(id, value, env = process.env) {
  if (!isOpenOcti(env) || id !== 'stripe') throw new Error('Unsupported installation integration')
  const store = readStore(env, { strict: true })
  store.keys[`integration:${id}`] = encryptKey(JSON.stringify(value), env)
...
export function syncOpenOctiKeysToOpenClaw(env = process.env) {
  ...
  // The pinned OpenClaw 2026.6.x gateway hot-reloads provider settings but can
  // retain an empty per-agent model registry after the first key is added.
  // Its supported restart mode rebuilds those registries automatically.
  config.gateway = { ...(config.gateway || {}), reload: { ...(config.gateway?.reload || {}), mode: 'restart' } }
  if (selected) {
