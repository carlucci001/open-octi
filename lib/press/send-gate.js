...
export function buildComplianceFooter(physicalAddress, unsubscribeUrl, options = {}) {
  ...
  return [
    ...
    options.outreach ? 'Sent by Your organization. ' : 'This editorial pitch was sent by Your organization. ',
    address.replace(/[<>&"]/g, ''),
    '. <a href="' + url.replace(/"/g, '%22') + '">' + (options.outreach ? 'Unsubscribe' : 'Unsubscribe from future pitches') + '</a>.',
    '</p>',
  ].join('')
}

export function newUnsubscribeToken() { return randomUUID() }

// Shared token matching/consumption pattern; outreach retains consumed tokens
// so mail scanners and repeated one-click POSTs get an idempotent success.
export function consumeUnsubscribeRecord(records, token, { repeatable = false, now = new Date().toISOString() } = {}) {
  const record = records.find(item => (item.token || item.unsubscribeToken) === token && (repeatable || !item.usedAt))
...
export function consumePressUnsubscribeToken(token) {
  return mutateData('press-unsubscribe-tokens.json', current => {
    ...
    const record = consumeUnsubscribeRecord(tokens, token)
    return { data: current || { tokens }, result: record }
  })
}
...
function releaseContent(campaign, args) {
  ...
  return {
    subject: String(args.subject || campaign.subject || document.title || 'News from Your organization').trim(),
