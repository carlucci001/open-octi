...
export async function createPressCheckout({ stripe, key, account, tenantId, planId, origin, testDouble = false, nodeEnv = process.env.NODE_ENV } = {}) {
  ...
  if (stripeAccount?.id !== PRESS_STRIPE_ACCOUNT_ID) throw new Error('Stripe key is not connected to the Your organization account')
...
export async function processPressOrderEvent(event, options = {}) {
  ...
  if (liveEvent) {
    ...
    if (stripeAccount?.id !== PRESS_STRIPE_ACCOUNT_ID) throw new Error('Live-mode press order event is not connected to the Your organization account')
  }
