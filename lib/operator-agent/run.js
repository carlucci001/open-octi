...
import { executeOperatorTool } from './tools/common.js'
...
export async function runOperatorTurn({
  ...
  const execute = async call => {
    ...
    try {
      const result = await withToolTimeout(() => executeOperatorTool(selected, call.input || {}, { request, conversationId, user: operatorContext.user }), timeoutMs)
