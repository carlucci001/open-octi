...
// Shared boundary for local Operator and commercial support execution. Keep
// this in approved common source so the OpenOcti snapshot has no private import.
export async function executeOperatorTool(selected, input, context = {}) {
  context.signal?.throwIfAborted()
  await context.checkpoint?.()
  const result = await selected.execute(input || {}, context)
  await context.checkpoint?.()
  context.signal?.throwIfAborted()
  return result
}
