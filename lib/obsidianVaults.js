...
function configuredVaultRoot() {
  const explicit = process.env.COMMAND_VAULT_ROOT || process.env.DEV_ROOT
  if (explicit) return normalizePath(explicit)
  const dataDir = process.env.CRM_DATA_DIR || path.join(process.cwd(), 'data')
  return normalizePath(path.join(dataDir, 'vaults'))
}

function linuxMirrorForWindowsPath(value) {
  const clean = normalizePath(value)
  const m = clean.match(/^[a-z]:\/dev\/(.+)$/i)
  return m ? normalizePath(path.join(configuredVaultRoot(), m[1])) : ''
}

function rootMirrorForDevPath(value) {
  const clean = normalizePath(value)
  const windows = clean.match(/^[a-z]:\/dev\/(.+)$/i)
  if (windows) return normalizePath(path.join(configuredVaultRoot(), windows[1]))
  const homeDev = clean.match(/^\/home\/[^/]+\/dev\/(.+)$/i)
  return homeDev ? normalizePath(path.join(configuredVaultRoot(), homeDev[1])) : ''
}
...
export function getVaults() {
  ...
  if (isOpenOcti() && !cfg.vaultPath && !cfg.vaults?.length) ensureStarterVault()
  const configured = Array.isArray(cfg.vaults) && cfg.vaults.length
    ? cfg.vaults
    : cfg.vaultPath
      ? [{ id: 'default', name: cfg.vaultName || 'Vault', path: cfg.vaultPath }]
      : [{ id: 'default', name: isOpenOcti() ? 'Command Vault' : 'Vault', path: path.join(configuredVaultRoot(), 'default') }]

  const vaults = configured.map(v => {
  ...
  const leadSourcePath = path.join(process.cwd(), 'vault', 'lead-sources')
  if (fs.existsSync(leadSourcePath) && !vaults.some(v => v.id === 'lead-sources')) {
    vaults.push({
      id: 'lead-sources',
      name: 'Lead Sources',
      path: leadSourcePath,
      configuredPath: leadSourcePath,
      available: true,
    })
  }
  return vaults
