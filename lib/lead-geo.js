...
function resolveCity(city, hintAbbrev, defaultState) {
  ...
  if (hintAbbrev) {
    ...
    if (!zips) return null // "City, ST" — that city is not in that state
...
//   "City, ST"                                  -> that city's ZIPs
//   "City, ST" / "City, STrth Carolina"
//   "City, ST North Carolina" (no comma)
...
export function resolveLocationToZips(location, opts = {}) {
  ...
  // "City, State" (ignore any trailing ZIP parts like "City, ST, 28801").
