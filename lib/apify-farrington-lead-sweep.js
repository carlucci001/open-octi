...
import { formatNewBusinessDigest, NEW_BUSINESS_LIST_ID } from './new-business-daily'
import { findExistingLeadMatch } from './leadDedupe'
import { signalSinceDate } from './lead-signals/adapters/common'
import { getCred } from './agent-creds'
import { create, loadAll, update } from './entityStore'
import { buildFarringtonLeadQuery, FARRINGTON_LEAD_VERTICALS, getFarringtonLeadVertical } from './farrington-lead-verticals'
import { domainOf, extractContacts, findBusinesses, pullSignals, resolveLeadVendorConfig, stateAbbrev } from './lead-vendors'
...
function leadIdentityKeys(lead) {
  const keys = []
  if (lead.signal?.sourceId) {
    for (const externalId of [lead.signal?.externalId, ...(lead.signal?.externalIds || [])].filter(Boolean)) {
      keys.push(`signal:${lead.signal.sourceId}:${externalId}`)
    }
    const org = String(lead.businessName || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()
    const address = String(lead.address || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()
    const phone = String(lead.phone || '').replace(/\D/g, '').slice(-10)
    if (org && address) keys.push(`signal-org:${org}|${address}|${phone}`)
    // Campaign compliance firms commonly share an email address and phone
    // across unrelated committees. The official committee id is authoritative;
    // contact reuse must not collapse separate campaign organizations.
    if (lead.signal.trigger === 'campaign') return [...new Set(keys)]
  }
  const contactKeys = [lead.email, lead.phone].map(identityKey).filter(Boolean)
  keys.push(...contactKeys)
  if (!contactKeys.length) keys.push(...[lead.website, lead.sourceUrl].map(identityKey).filter(Boolean))
  return [...new Set(keys)]
}

function mergeExternalIds(target, source) {
  if (!target?.signal?.sourceId || target.signal.sourceId !== source?.signal?.sourceId) return false
  const externalIds = [...new Set([
    target.signal.externalId,
    ...(target.signal.externalIds || []),
    source.signal.externalId,
    ...(source.signal.externalIds || []),
  ].filter(Boolean))]
  target.signal = { ...target.signal, externalIds }
  return true
}

export function selectLeadDrafts({ drafts = [], existingLeads = [], limit = 10, requireEmail = false } = {}) {
  const keyOwners = new Map()
  for (const lead of Array.isArray(existingLeads) ? existingLeads : []) {
    for (const key of leadIdentityKeys(lead)) keyOwners.set(key, { kind: 'existing', lead })
  }
  const toCreate = []
  const skipped = []
  const externalIdUpdates = new Map()
  const skipReasons = { duplicate: 0, missingContact: 0, missingIdentity: 0, missingEmail: 0 }
  let reviewed = 0
  ...
  for (const draft of Array.isArray(drafts) ? drafts : []) {
    ...
    const sharedMatch = draft.signal?.trigger === 'new-business' ? findExistingLeadMatch(draft, [...existingLeads, ...toCreate], { businessOnly: true }) : null
    const duplicateOwner = keys.map(key => keyOwners.get(key)).find(Boolean)
      || (sharedMatch ? { kind: existingLeads.includes(sharedMatch.lead) ? 'existing' : 'draft', lead: sharedMatch.lead } : null)
    if (duplicateOwner) {
      if (mergeExternalIds(duplicateOwner.lead, draft) && duplicateOwner.kind === 'existing') {
        externalIdUpdates.set(duplicateOwner.lead.id, duplicateOwner.lead.signal.externalIds)
      }
      skipReasons.duplicate += 1
      skipped.push({ businessName: draft.businessName, website: draft.website, reason: 'duplicate' })
      continue
    }
    const mailContactable = draft.signal?.channels?.includes('mail') && Boolean(draft.address)
    if (!draft.phone && !draft.email && !mailContactable) {
    ...
    // Carl directive 2026-09-14: a lead with no email address is not accepted
    // when the automation sets dataSource.requireEmail (default on for
    // the New business owners daily sweep). Address-only registry rows
    // were flooding the pipeline and the digest.
    if (requireEmail && !String(draft.email || '').trim()) {
      skipReasons.missingEmail += 1
      skipped.push({ businessName: draft.businessName, website: draft.website, reason: 'missing email' })
      continue
    }

    if (draft.signal) mergeExternalIds(draft, draft)
    keys.forEach(key => keyOwners.set(key, { kind: 'draft', lead: draft }))
    toCreate.push(draft)
  }
  ...
  return {
    ...
    externalIdUpdates: [...externalIdUpdates].map(([id, externalIds]) => ({ id, externalIds })),
  }
}
...
export function leadFromApifyResult(item, { now, campaign, leadListId = null, location, query, runId, spec, vendor, vertical }) {
  const signal = item.__signal || null
  const publicAgency = signal?.provenance?.agency || (/^fec-/i.test(signal?.sourceId || '') ? 'FEC' : null)
  const publicSource = publicAgency ? `government_public_record_${String(publicAgency).toLowerCase()}` : 'government_public_record'
  ...
  const email = signal ? String(item.email || '') : extractEmail(item)
  const phone = signal ? normalizePhone(item.phone || '') : extractPhone(item)
  ...
  const contactTitle = firstText(item, ['contactTitle', 'title', 'contact.title'])
  ...
  return {
    ...
    website: link,
    sourceUrl: link,
    source: signal ? publicSource : (vendor.provider === 'apify' ? 'apify_google_maps' : `${vendor.provider}_lead_finder`),
    leadSourceProvider: signal ? 'public-record' : vendor.provider,
    leadSourceActor: signal ? signal.sourceId : vendor.finderActorId,
    ...
    tags: [
      ...
      signal?.trigger === 'new-business' ? 'new-business' : null,
      signal ? `signal:${signal.sourceId}` : null,
      signal?.state ? `state:${signal.state}` : null,
      signal?.triggeredAt ? `filed:${signal.triggeredAt.slice(0, 10)}` : null,
      signal ? `signal:${signal.trigger}` : 'cold-call',
      signal ? signal.sourceId : vendor.provider,
      campaign,
      vertical.id,
      locationTag,
      sourceDateTag,
      signal?.sourceId === 'cslb-ca-contractors' ? 'signal:contractor' : null,
      signal?.sourceId === 'cslb-ca-contractors' ? 'contractors-ca' : null,
      signal?.sourceId === 'cslb-ca-contractors' ? `trigger:${signal.trigger}` : null,
    ].filter(Boolean),
    signal: signal ? { sourceId: signal.sourceId, externalId: signal.externalId, externalIds: signal.externalIds || [signal.externalId], trigger: signal.trigger, triggeredAt: signal.triggeredAt, state: signal.state, address: signal.address, channels: signal.channels, attrs: signal.attrs, people: signal.people } : undefined,
    provenance: signal?.provenance || undefined,
    notes: [
      ...
      signal?.attrs?.cashOnHand !== undefined ? `Cash on hand: $${Number(signal.attrs.cashOnHand || 0).toLocaleString('en-US')}` : '',
      signal?.attrs?.receipts !== undefined ? `Receipts: $${Number(signal.attrs.receipts || 0).toLocaleString('en-US')}` : '',
      signal?.attrs?.lastReport ? `Last FEC report: ${signal.attrs.lastReport}` : '',
      signal?.attrs?.licenseNo ? `License: ${signal.attrs.licenseNo}` : '',
      signal?.attrs?.classifications?.length ? `Classification: ${signal.attrs.classifications.join(', ')}` : '',
      signal?.attrs?.triggerDate ? `Trigger date (${signal.trigger}): ${String(signal.attrs.triggerDate).slice(0, 10)}` : '',
      signal?.people?.length ? `Public-record contacts: ${signal.people.map(person => `${person.name} (${person.title})`).join('; ')}` : '',
      snippet ? `Search signal: ${snippet}` : '',
      `Query: ${query}`,
      spec?.notes ? `Spec notes: ${spec.notes}` : '',
    ].filter(Boolean).join('\n'),
    legacy: {
      source: signal ? 'public-record' : (vendor.provider === 'apify' ? 'apify-google-maps' : `${vendor.provider}-lead-finder`),
      campaign,
...
function buildEmailHtml({ created, skipped, skipReasons, query, location, vertical }) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827;max-width:840px;margin:0 auto;padding:24px;">
      <h1 style="font-size:22px;margin:0 0 8px;">OpenOcti lead sweep: ${escapeHtml(vertical.label)}</h1>
      <p style="margin:0 0 16px;color:#4b5563;">The sweep imported ${created.length} contactable Your organization cold-call lead${created.length === 1 ? '' : 's'} and skipped ${skipped.length} source${skipped.length === 1 ? '' : 's'} after duplicate, contact, and identity checks${skippedBreakdown ? ` (${escapeHtml(skippedBreakdown)})` : ''}.</p>
...
export function buildLeadSweepEmailText({ created, skipped, skipReasons, query, location, vertical }) {
  ...
  const skippedBreakdown = formatLeadSkipReasons(skipReasons)
  return [
    `OpenOcti lead sweep completed: ${vertical.label}`,
    `Location: ${location}`,
    `Query: ${query}`,
...
function resolveRunConfig(automation = {}) {
  ...
  const rawLocation = String(rotation?.location || dataSource.location || automation.location || 'United States').trim()
  const location = vertical.id === 'new-businesses' && rawLocation.toUpperCase() === 'UNITED STATES' ? 'US' : rawLocation
  ...
  const campaign = String(campaignRaw || (vertical.id === 'new-businesses' ? 'fd-new-business-daily' : `fd-cold-${vertical.id}`)).trim()
  ...
  const signalOptions = dataSource.signalOptions || automation.signalOptions || {}
  ...
  return { campaign, enrichContacts, leadListId, limit, location, query, rotation, scrapeContacts, signalOptions, spec, vendor, vertical }
}

async function sendSummaryEmail({ automation, created, skipped, skipReasons, query, location, recipientEmail, vertical, sources, errors, enrichment }) {
  const daily = vertical.id === 'new-businesses'
  const to = String(daily ? process.env.LEAD_SWEEP_DIGEST_TO || process.env.RESEND_REPLY_TO || 'personal@example.invalid' : recipientEmail || automation.delivery?.recipients?.[0] || '').trim()
  if (!to || !to.includes('@') || (!daily && !hasOutboundEmailTransport())) return { emailedTo: null, emailId: null }
  if (daily) {
    const { outreachYesterday } = await import('./outreach-store')
    const message = formatNewBusinessDigest({ leads: created, skipped: skipped.length, sources, errors, enrichment, outreach: outreachYesterday() })
    const data = await sendOutboundEmail({ to, ...message })
    return { emailedTo: to, emailId: data?.id || null }
  }
  const subject = `OpenOcti ${vertical.label} lead sweep: ${created.length} new`
  const data = await sendOutboundEmail({
...
export function placesLookupsPerRun(value = process.env.LEAD_PLACES_LOOKUPS_PER_RUN) {
  const cap = value === undefined || value === '' ? 25 : Number(value)
  return Number.isFinite(cap) && cap >= 0 ? Math.floor(cap) : 25
}

export async function enrichDraftContacts(drafts, { vendor, apifyKey }) {
  const summary = { requested: 0, sitesWithEmail: 0, phonesAdded: 0, placesPhonesAdded: 0, websitesAdded: 0, placesLookups: 0, actorId: vendor.enrichActorId, error: null, unavailable: null }
  if (!apifyKey) return { ...summary, unavailable: 'missing-apify-credential' }

  // Enrich an existing public-record identity, never create a Places-sourced lead.
  // Exact normalized name + city/state avoids attaching a nearby namesake's phone.
  const identity = value => String(value || '').toLowerCase().replace(/[^a-z0-9]/g, '')
  const placesVendor = { ...vendor, provider: 'apify' }
  for (const draft of drafts) {
    if (summary.placesLookups >= placesLookupsPerRun()) break
    const address = draft.signal?.address
    if (!address?.city || !address?.state || (draft.phone && draft.website)) continue
    summary.placesLookups += 1
    try {
      const matches = await findBusinesses({ query: draft.businessName, location: `${address.city}, ${address.state}`, maxItems: 3, scrapeContacts: false }, placesVendor, apifyKey)
      const match = matches.find(item => identity(firstText(item, ['businessName', 'companyName', 'placeName', 'title', 'name'])) === identity(draft.businessName)
        && identity(item.city) === identity(address.city)
        && stateAbbrev(item.state) === stateAbbrev(address.state))
      if (!match) continue
      const phone = normalizePhone(firstText(match, ['phone', 'phoneNumber']))
      const website = firstText(match, ['website', 'websiteUrl'])
      if (!draft.phone && phone) { draft.phone = phone; summary.phonesAdded += 1; summary.placesPhonesAdded += 1 }
      if (!draft.website && domainOf(website)) { draft.website = website; summary.websitesAdded += 1 }
      draft.notes += `\nPlaces contact lookup: ${draft.businessName}, ${address.city}, ${address.state}`
      ...
      summary.error = error.message || 'Places contact lookup failed'
    }
  }
  ...
  for (const draft of drafts) {
    if ((draft.email && draft.phone) || !draft.website) continue
...
export async function runFarringtonLeadSweep(automation, options = {}) {
  const { campaign, enrichContacts, leadListId: requestedList, limit, location, query, rotation, scrapeContacts, signalOptions, spec, vendor, vertical } = resolveRunConfig(automation)
  const newBusiness = vertical.id === 'new-businesses'
  const apifyKey = getCred('apify')?.key
  const provenOnly = newBusiness || options.provenOnly || automation.dataSource?.provenOnly
  const leadListId = requestedList || (newBusiness ? NEW_BUSINESS_LIST_ID : null)
  if (newBusiness && !loadAll('leadLists').some(list => list.id === leadListId)) create('leadLists', { id: leadListId, name: 'New business owners', brandContext: 'farrington_dev', system: true })
  ...
  const signalLimit = signalPullLimitForVertical(vertical.id, limit)
  const signalSource = await pullSignals({
    leadType: vertical.id,
    location,
    resolvedSources: options.resolvedSignalSources,
    jurisdiction: options.signalJurisdiction || {},
    since: options.signalSince || (newBusiness ? signalSinceDate(!automation.lastRunAt && !existingLeads.some(lead => lead.campaign === campaign) ? undefined : automation.dataSource?.signalSince, { firstRun: !automation.lastRunAt }) : automation.dataSource?.signalSince),
    signalOptions,
    // Pull beyond the requested count because one public source can return
    // multiple records for the same organization (NPPES NPIs are a common
    // example). Selection below dedupes and then fills the requested count.
    limit: signalLimit,
  })
  const signalItems = signalSource.rows.map(row => ({
    businessName: row.entity.name,
    name: row.people?.[0]?.name || '',
    contactName: row.people?.[0]?.name || '',
    title: row.people?.[0]?.title || '',
    phone: row.entity.phone,
    email: row.entity.email,
    website: row.entity.website,
    address: [row.entity.address?.line1, row.entity.address?.city, row.entity.address?.state, row.entity.address?.zip].filter(Boolean).join(', '),
    __signal: {
      ...
      channels: signalSource.resolution?.sources?.find(source => source.id === row.sourceId)?.compliance?.channels || [],
      provenance: row.provenance,
      attrs: row.attrs,
      people: row.people,
      state: row.jurisdiction?.state || row.entity.address?.state,
      address: row.entity.address,
    },
  }))
  const requestedTotal = newBusiness && location.toUpperCase() === 'US' ? limit * (signalSource.resolution?.sources?.length || 0) : limit
  const fallbackLimit = Math.max(0, limit - signalItems.length)
  ...
  const source = !provenOnly && fallbackLimit > 0 && apifyKey ? await collectLeadSourceBatches({
    provider: vendor.provider,
    limit: fallbackLimit,
    ...
    fetchBatch: async batchIndex => {
      ...
      return findBusinesses({
        query: searchTerm,
        location,
        maxItems: Math.min(fallbackLimit * 4, 100),
        scrapeContacts,
        batchIndex,
      }, vendor, apifyKey)
    },
  ...
  const results = [...signalItems, ...uniqueApifyResults(source.items, sourceResultLimit)]
  ...
  const enrichment = enrichContacts
    ...
    : { requested: 0, sitesWithEmail: 0, phonesAdded: 0, placesPhonesAdded: 0, websitesAdded: 0, placesLookups: 0, actorId: null, error: null, unavailable: null, disabled: true }
  ...
  const requireEmail = automation.dataSource?.requireEmail ?? newBusiness
  const selection = selectLeadDrafts({ drafts, existingLeads: loadAll('leads'), limit: newBusiness && location.toUpperCase() === 'US' ? drafts.length : limit, requireEmail })
  ...
  for (const merged of selection.externalIdUpdates || []) {
    const existing = existingLeads.find(lead => lead.id === merged.id)
    if (existing) update('leads', merged.id, { signal: { ...(existing.signal || {}), externalIds: merged.externalIds } })
  }
  const created = toCreate.map(draft => create('leads', draft))

  emit({ step: 5, stepsTotal: 5, phase: 'summary', phaseLabel: 'Sending run summary' })

  const email = options.skipEmail ? { emailedTo: null, emailId: null } : await sendSummaryEmail({
    automation,
    created,
    skipped,
    ...
    vertical,
    sources: signalSource.sources,
    errors: signalSource.errors,
    enrichment,
  })
  ...
  return {
    campaign,
    runId,
    providerId: newBusiness ? 'public-record' : vendor.provider,
    actorId: newBusiness ? null : vendor.provider === 'apollo' ? vendor.peopleActorId : vendor.finderActorId,
    sourceBatches: source.batchesRun,
    signalSources: signalSource.sources,
    signalErrors: signalSource.errors,
    enrichment,
    ...
    ...(newBusiness ? { limitPerSource: limit } : {}),
    returned: results.length,
    created: created.length,
    withEmail: created.filter(lead => lead.email).length,
    withPhone: created.filter(lead => lead.phone).length,
    withWebsite: created.filter(lead => lead.website).length,
    sources: (signalSource.sources || []).map(source => ({
      sourceId: source.sourceId || source.id,
      rows: source.rows ?? source.returned ?? 0,
      created: created.filter(lead => lead.signal?.sourceId === (source.sourceId || source.id)).length,
    })),
    skipped: skipped.length,
    skipReasons: selection.skipReasons,
    reviewed: selection.reviewed,
    unprocessed: selection.unprocessed,
    shortfall: Math.max(0, requestedTotal - created.length),
...
export function signalPullLimitForVertical(verticalId, requestedLimit) {
  const limit = Math.max(1, Number(requestedLimit) || 10)
  return ['political-campaigns', 'new-businesses'].includes(verticalId) ? limit : Math.min(Math.max(limit * 4, 50), 200)
}
