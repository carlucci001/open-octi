...
function leadFromApifyResult(item, { now, campaign, leadListId = null, location, query, runId, spec, vendor, vertical }) {
  ...
  return {
    ...
    website: link,
    sourceUrl: link,
    source: signal ? 'government_public_record' : (vendor.provider === 'apify' ? 'apify_google_maps' : `${vendor.provider}_lead_finder`),
    ...
    tags: ['farrington-development', signal ? `signal:${signal.trigger}` : 'cold-call', signal ? signal.sourceId : vendor.provider, campaign, vertical.id, locationTag, sourceDateTag],
...
export async function runFarringtonLeadSweep(automation, options = {}) {
  ...
  const source = fallbackLimit > 0 && apifyKey ? await collectLeadSourceBatches({
    provider: vendor.provider,
    limit: fallbackLimit,
