...
import { domainOf, extractContacts, findBusinesses, pullSignals, resolveLeadVendorConfig } from './lead-vendors'
...
export function leadFromApifyResult(item, { now, campaign, leadListId = null, location, query, runId, spec, vendor, vertical }) {
  ...
  return {
    ...
    signal: signal ? { sourceId: signal.sourceId, externalId: signal.externalId, externalIds: signal.externalIds || [signal.externalId], trigger: signal.trigger, triggeredAt: signal.triggeredAt, channels: signal.channels, attrs: signal.attrs, people: signal.people } : undefined,
    ...
    notes: [
      ...
      signal?.people?.length ? `Campaign contacts: ${signal.people.map(person => `${person.name} (${person.title})`).join('; ')}` : '',
...
export async function runFarringtonLeadSweep(automation, options = {}) {
  ...
  const { campaign, enrichContacts, leadListId, limit, location, query, rotation, scrapeContacts, signalOptions, spec, vendor, vertical } = resolveRunConfig(automation)
  ...
  const source = !options.provenOnly && fallbackLimit > 0 && apifyKey ? await collectLeadSourceBatches({
    provider: vendor.provider,
    limit: fallbackLimit,
...
export function signalPullLimitForVertical(verticalId, requestedLimit) {
  ...
  return verticalId === 'political-campaigns' ? limit : Math.min(Math.max(limit * 4, 50), 200)
}
