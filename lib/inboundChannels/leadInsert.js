...
function brandContextForChannel(channel) {
  if (channel.id.startsWith('myvtc')) return 'myvtc'
  if (channel.id === 'newsroomaios_demos') return 'newsroomaios'
  if (channel.id === 'wnc_times') return 'wnc_times'
...
export async function insertLeadFromChannel({ channel, payload, externalId, sourceMeta }) {
  ...
  // (newsroomaios_demos → NewsroomAIOS calendar, fd-concierge/fd_inquiries → Farrington Dev calendar).
  ...
  if (whenField) {
    try {
      const result = await createAppointmentEvent({
        source: channel.id,
        when: whenField,
        kind: channel.id === 'newsroomaios_demos' ? 'demo' : (channel.id === 'fd-concierge' ? 'callback' : 'meeting'),
