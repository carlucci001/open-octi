...
function serviceLineForChannel(channel, payload, sourceMeta) {
  return sourceMeta?.serviceLine ||
    ...
    (brandContextForChannel(channel) === 'farrington_dev' ? 'Your organization Services' : '')
}
...
export async function insertLeadFromChannel({ channel, payload, externalId, sourceMeta }) {
  ...
  // (ContentStudio_demos → ContentStudio calendar, fd-concierge/fd_inquiries → Business Development calendar).
