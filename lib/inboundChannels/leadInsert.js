...
function brandContextForChannel(channel) {
  ...
  if (channel.id === 'ContentHub_demos') return 'ContentHub'
...
export async function insertLeadFromChannel({ channel, payload, externalId, sourceMeta }) {
  ...
  // (ContentHub_demos → ContentHub calendar, fd-concierge/fd_inquiries → Farrington Dev calendar).
  ...
  if (whenField) {
    try {
      const result = await createAppointmentEvent({
        source: channel.id,
        when: whenField,
        kind: channel.id === 'ContentHub_demos' ? 'demo' : (channel.id === 'fd-concierge' ? 'callback' : 'meeting'),
