...
export async function testIntegrationConnection(id, env = process.env) {
  ...
  if (id === 'postiz') {
    ...
    if (!config || config.error) return { ok: false, message: config?.error || 'Connect a Postiz server and API key first.' }
    try {
      ...
      return { ok: true, message: `Postiz connection verified. ${channels.length} connected channel(s). Publishing has not been tested.` }
    } catch {
      return { ok: false, message: 'Postiz could not be verified. Check the server, Public API URL, and API key.' }
    }
  }
