...
import { readData, writeData, mutateData } from '@/lib/dataStore'
...
export function normalizeLicense(record = {}, index = 0) {
  ...
  return {
    ...
    orderId: cleanString(record.orderId, '', 160),
    modules: Array.isArray(record.modules) ? [...new Set(record.modules.map(v => cleanId(v)).filter(Boolean))] : [],
    updatesUntil: cleanString(record.updatesUntil, '', 80),
    supportUntil: cleanString(record.supportUntil, '', 80),
    stripeSubscriptionId: cleanString(record.stripeSubscriptionId, '', 160),
    lastSupportInvoiceId: cleanString(record.lastSupportInvoiceId, '', 160),
    delivery: record.delivery && typeof record.delivery === 'object' ? record.delivery : {},
    downloads: Array.isArray(record.downloads) ? record.downloads.slice(-100) : [],
...
export function upsertLicense(record) {
  return mutateData(LICENSE_FILE, raw => {
    const store = { ...raw, licenses: (raw?.licenses || []).map(normalizeLicense), updatedAt: new Date().toISOString() }
    const duplicate = record.productId === 'octi-cc' && record.orderId && store.licenses.find(item => item.productId === 'octi-cc' && item.orderId === record.orderId && item.id !== record.id)
    if (duplicate) return { data: store, result: store }
    const normalized = normalizeLicense({ ...record, updatedAt: store.updatedAt })
...
export function mutateLicense(id, update) {
  return mutateData(LICENSE_FILE, raw => {
    const store = { ...raw, licenses: (raw?.licenses || []).map(normalizeLicense), updatedAt: new Date().toISOString() }
    const license = store.licenses.find(item => item.id === id)
    const result = license ? update(license) : null
    return { data: store, result }
  })
}

export function recordLicenseDownload(licenseKey, entry) {
  const fingerprint = keyFingerprint(licenseKey)
  const license = getLicenseStore().licenses.find(item => item.keyFingerprint === fingerprint)
  if (!license) return
  mutateLicense(license.id, item => {
    item.downloads = [...item.downloads, { at: new Date().toISOString(), ...entry }].slice(-100)
  })
}
