...
export function parseCreditTopUpRequest(body) {
  ...
  const allowed = new Set(['packId', 'requestId'])
  if (Object.keys(body).some(key => !allowed.has(key))) {
    throw new Error('Only packId and requestId are accepted')
  }
...
export function buildCreditTopUpIntent({
  ...
  const resolvedPack = getCreditTopUpPack(pack?.id)
  if (!resolvedPack) throw new Error('Unknown credit pack')
  const parsed = parseCreditTopUpRequest({ packId: resolvedPack.id, requestId })
  const identity = {
  ...
  return {
    ...
    params: {
      ...
      description: `${resolvedPack.name} credit pack (${resolvedPack.credits.toLocaleString('en-US')} credits)`,
      metadata: {
...
export function validateCreditTopUpPaymentIntent(intent, lease) {
  ...
  const pack = getCreditTopUpPack(intent.metadata.packId)
  if (!pack || String(intent.metadata.credits || '') !== String(pack.credits)) {
    throw new Error('Credit top-up pack mismatch')
  }
  const { requestId } = parseCreditTopUpRequest({ packId: pack.id, requestId: intent.metadata.requestId })
