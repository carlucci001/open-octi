...
import { listOpenOctiKeyStatus } from './openocti-keys'
import { normalizeImageGenerationPreference } from './image-generation-preferences'

function detectKeysCombined(openclawCfg) {
  const out = new Set()
  if (isOpenOcti()) {
    for (const provider of listOpenOctiKeyStatus()) {
      if (provider.status === 'configured') out.add(provider.id === 'gemini' ? 'google' : provider.id)
    }
  }
...
function mergeAgentView(remoteAgent, localMeta) {
  ...
  const avatar = getAvatarMeta(id, meta.avatar || meta.avatarUrl)
...
export async function listAgents() {
  ...
  try {
    ...
    const directProviderReady = ['openai', 'anthropic', 'google', 'openrouter', 'orcarouter'].some(provider => availableProviders.has(provider))
    ...
    for (const [id, meta] of Object.entries(local.agents || {})) {
      ...
      fallbackAgents.push({
        ...
        title: meta.title || preset.title || '',
        voiceProfile: meta.voiceProfile || preset.voiceProfile || '',
        avatar: getAvatarMeta(id, meta.avatar || meta.avatarUrl) || null,
        avatarPrompt: meta.avatarPrompt || preset.avatarPrompt || '',
    ...
    for (const preset of PRESETS) {
      ...
      fallbackAgents.push({
        ...
        avatar: getAvatarMeta(preset.id, preset.avatar || preset.avatarUrl) || null,
