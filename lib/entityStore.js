...
const FILES = {
  outreachMessages: { file: 'outreach.json', key: 'outreachMessages', prefix: 'om' },
  outreachSuppressions: { file: 'outreach.json', key: 'outreachSuppressions', prefix: 'os' },
  ...
  pressOutlets:   { file: 'press-outlets.json',   key: 'pressOutlets',   prefix: 'po' },
  pressContacts:  { file: 'press-contacts-v2.json', key: 'pressContacts', prefix: 'pc' },
  pressBylines:   { file: 'press-bylines.json',   key: 'pressBylines',   prefix: 'pby' },
  pressBeats:     { file: 'press-beats.json',     key: 'pressBeats',     prefix: 'pbt' },
  pressLists:     { file: 'press-lists.json',     key: 'pressLists',     prefix: 'plst' },
  pressCampaigns: { file: 'press-campaigns.json', key: 'pressCampaigns', prefix: 'pcmp' },
  pressSuppression: { file: 'press-suppression.json', key: 'pressSuppression', prefix: 'psup' },
  leadSources:    { file: 'lead-sources.json',     key: 'leadSources',    prefix: 'lsrc' },
  sourceValidations: { file: 'source-validations.json', key: 'sourceValidations', prefix: 'sval' },
  leadSignals:    { file: 'lead-signals.json',     key: 'leadSignals',    prefix: 'lsig' },
...
export function saveAll(type, list) {
  ...
  if (file === 'outreach.json') return mutateData(file, current => ({ data: { ...current, [key]: list }, result: true }))
...
// Returns the number of records actually removed (not a boolean) so callers
// that report a count - e.g. the Lead Manager bulk-delete notice (WO-LB1) -
// get an accurate number instead of Number(true) always collapsing to 1.
