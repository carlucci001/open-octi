...
export function recordProductTermsRequest(order = {}) {
  ...
  return create('leads', {
    ...
    status: 'new',
    notes: [
      `${order.paymentOptionLabel || 'Payment terms review'} requested for ${order.productName || 'OpenOcti'} ${order.packageName || ''}.`,
    ...
    productOpportunity: clean(order.productName || 'OpenOcti', 160),
    serviceLine: 'Your organization - Command Center',
...
export function ensurePaidProductOnboardingTask(order = {}) {
  ...
  return create('tasks', {
    ...
    description: [
      ...
      `${order.productName || 'OpenOcti'} - ${order.packageName || 'package'}.`,
