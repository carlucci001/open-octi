...
const OPENCLAW_ACTION_TOOL = isOpenOcti() ? 'the exact attached named tool in OpenClaw' : 'fcc_call in OpenClaw'
...
- Execute the selected operation with crm_action (or ${OPENCLAW_ACTION_TOOL}). Never claim success until the tool returns a successful result. When a request asks for contacts, records, or research details, return the actual requested fields rather than only category or publication names.
