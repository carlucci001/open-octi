...
async function geminiNarrative(facts, pagespeed, scores, types) {
  ...
  const prompt = [
    `You are writing the narrative for a ${typeLabels} website report prepared by Your organization for a client.`,
...
function renderReportHtml({ facts, pagespeed, scores, narrative, narrativeModel, types, accountName, generatedAt }) {
  ...
  return [
    ...
    `<p style="text-transform:uppercase;letter-spacing:0.08em;font-size:11px;color:${INK_MUTED};margin:0 0 4px;">Your organization · Website Intelligence</p>`,
