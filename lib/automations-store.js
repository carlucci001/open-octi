...
import { NEW_BUSINESS_DAILY, NEW_BUSINESS_LIST_ID } from './new-business-daily'
import { isOpenOcti } from './edition'
import { OUTREACH_AUTOMATION } from './outreach-config'

const FILE = 'automations.json'
const VERSION = 1

function load() {
  const stored = readData(FILE)
  const data = stored?.__version ? stored : { __version: VERSION, automations: [] }
  if (!isOpenOcti() && !data.ls7Seeded) {
    if (!data.automations.some(item => item.id === OUTREACH_AUTOMATION.id)) data.automations.push({ ...structuredClone(OUTREACH_AUTOMATION), createdAt: nowISO(), updatedAt: nowISO() })
    data.ls7Seeded = true
    save(data)
  }
  if (!isOpenOcti() && !data.ls6Seeded) {
    if (!data.automations.some(item => item.id === NEW_BUSINESS_DAILY.id)) data.automations.push({ ...structuredClone(NEW_BUSINESS_DAILY), createdAt: nowISO(), updatedAt: nowISO() })
    const lists = readData('lead-lists.json') || { leadLists: [] }
    if (!lists.leadLists.some(list => list.id === NEW_BUSINESS_LIST_ID)) {
      lists.leadLists.push({ id: NEW_BUSINESS_LIST_ID, name: 'New business owners', brandContext: 'farrington_dev', campaignId: 'fd-new-business-daily', system: true, createdAt: nowISO() })
      writeData('lead-lists.json', lists)
    }
    data.ls6Seeded = true
    save(data)
  }
