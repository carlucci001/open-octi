...
const ALL_COMMAND_CENTER_SECTIONS = [
  {
    id: 'dashboard',
  ...
  },
  {
    id: 'press-desk',
    label: 'Press Desk',
    aliases: ['press desk', 'open press contacts', 'press contacts', 'press lists', 'journalists', 'media contacts', 'press campaigns', 'press release distribution'],
  },
  {
    id: 'lead-intake',
  ...
  },
  {
    id: 'platforms',
    label: 'Platforms',
    // SearchTools3's old aliases migrate here so voice/nav keeps working — they
    // land on Platforms, where SearchTools3 is the first platform in the list.
    aliases: ['platforms', 'platform', 'platform manager', 'platform list', 'managed platforms', 'platform control', 'SearchTools3', 'get found 3', 'get found three', 'visibility reports', 'seo reports', 'aeo reports', 'geo reports', 'website visibility', 'remediation reports'],
  },
  {
    id: 'contacts',
    label: 'Contacts',
  ...
  {
    id: 'migrate',
    label: 'System > Import & migrate',
    aliases: ['import', 'import data', 'import and migrate', 'import & migrate', 'migration', 'migration center', 'quick import'],
  },
  {
    id: 'settings',
    label: 'Settings',
...
const OPENOCTI_CLOSED_SECTION_IDS = new Set(['platforms'])

export function commandCenterSectionsFor(env = process.env) {
  if (!isOpenOcti(env)) return ALL_COMMAND_CENTER_SECTIONS
  const services = ALL_COMMAND_CENTER_SECTIONS.find(section => section.id === 'control-services')
  return ALL_COMMAND_CENTER_SECTIONS
    .filter(section => !OPENOCTI_CLOSED_SECTION_IDS.has(section.id) && section.id !== 'control-services')
    .map(section => section.id === 'settings' ? {
      ...section,
      label: 'System > Admin',
      aliases: [...section.aliases, 'admin', 'control-services', ...services.aliases],
    } : section)
}

export const COMMAND_CENTER_SECTIONS = Object.freeze(commandCenterSectionsFor())
...
export function labelForCommandCenterTab(value) {
  const id = resolveCommandCenterTab(value)
  if (!id) return ''
