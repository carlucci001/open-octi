...
export async function pullSignals({ leadType = '', location = '', resolvedSources, jurisdiction = {}, since, limit = 50, persist = true, signalOptions = {} } = {}) {
  ...
  const national = resolution.jurisdiction?.country === 'US' || String(location).toUpperCase() === 'US'
  ...
  for (const source of selectedSources) {
    if (!national && rows.length >= limit) break
    try {
      const sourceJurisdiction = national ? { state: source.coverage?.[0]?.slice(0, 2) } : resolution.jurisdiction || jurisdiction
      const result = await pullLeadSignals({ manifest: source, jurisdiction: sourceJurisdiction, since, limit: national ? limit : limit - rows.length, signalOptions })
      ...
      for (const row of result.rows) {
        ...
        returned.add(key)
        row.jurisdiction = sourceJurisdiction
        rows.push(row)
