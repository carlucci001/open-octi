...
export const INTEGRATION_DIRECTORY = Object.freeze({
  ...
  orcarouter: entry('Routes supported text and reasoning tasks through OrcaRouter. Uses its own API key.', 'https://www.orcarouter.ai/console', 'Free routing is available; paid routes depend on your OrcaRouter account and selected model.'),
  ...
  stripe: entry('Connects the payment terminal and invoice checkout. Products and recurring subscriptions are managed in Stripe Dashboard; automatic catalog sync is not included.', 'https://dashboard.stripe.com/apikeys', 'Transaction fees and account requirements apply. Configure through Admin → Stripe.'),
  ...
  postiz: entry('Connect a separately installed or hosted Postiz server, then link your social accounts in Postiz. The standard OpenOcti Docker stack does not install Postiz.', 'https://docs.postiz.com/self-host/installation/overview', 'Postiz is open source. Hosting and social-provider requirements are separate.'),
