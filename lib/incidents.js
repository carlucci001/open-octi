...
import { getOwnerUser, getOwnerUsername } from './auth'
...
export function createIncidentTask(incident, {
  ...
  now = new Date().toISOString(),
  getOwner = () => ({ username: getOwnerUsername(), displayName: getOwnerUser()?.displayName || getOwnerUsername() }),
} = {}) {
  ...
  const task = createTask('tasks', {
    ...
    assignedTo: owner.displayName,
    assignedToUserId: owner.username,
    completedAt: null,
  })
