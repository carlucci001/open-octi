...
import { ADDITIONAL_COMMAND_CENTER_MODULES, createOctiCcProduct } from '@/lib/octiccCatalog'
...
export const DEFAULT_PAYMENT_OPTIONS = [
  ...
  },
  {
    ...
    copy: 'Send Your organization a financing request for follow-up. This does not submit an application to a lender or promise approval, terms, or availability.',
  },
  {
    id: 'in-house-milestones',
    name: 'Request a milestone plan',
    copy: 'Ask Your organization to review a milestone payment structure. No plan is active until both sides approve written terms.',
  },
  {
    id: 'lease-to-own',
    name: 'Request lease-to-own review',
    copy: 'Ask Your organization to review a possible lease-to-own structure. Availability and terms are not guaranteed, and nothing is activated by this request.',
  },
]
...
export const DEFAULT_LICENSE_TEMPLATES = [
  {
    ...
    copy: 'Buyer receives source code for the licensed deployment while Your organization retains ownership of reusable platform IP, frameworks, templates, and underlying know-how.',
  ...
  },
  {
    ...
    copy: 'Customer receives hosted access to the managed Command Center service. Your organization operates the platform; customer receives use rights, configuration, support, and agreed service modules, but not source redistribution rights.',
...
const COMMAND_CENTER_MODULES = [
  { id: 'crm', name: 'CRM Core', price: 0, quote: false, copy: 'Accounts, contacts, leads, opportunities, tasks, notes, and activity timelines.' },
  { id: 'pipelines', name: 'Pipeline Management', price: 0, quote: false, copy: 'Lead stages, opportunity movement, qualification, forecasting, and follow-up control.' },
  { id: 'projects', name: 'Project Management', price: 0, quote: false, copy: 'Client projects, delivery states, implementation handoffs, and work tracking.' },
  { id: 'automation-starter', name: 'Automation Starter', price: 7500, quote: false, copy: 'Start with one practical automation path, then connect it to Command Center records, tasks, and follow-up.' },
  { id: 'ai-staff', name: 'AI Staff Manager', price: 0, quote: true, copy: 'Manage internal agents, prompts, tools, voice settings, and operating roles.' },
  { id: 'customer-agent-manager', name: 'Own Agent Manager', price: 12500, quote: false, copy: 'Give the buyer their own Agent Manager inside their own Command Center so automations can grow into managed agents.' },
  { id: 'automation-to-agent-migration', name: 'Automation-to-agent Migration', price: 0, quote: true, startingAt: 15000, copy: 'Move from a first automation into customer-owned agents, tools, prompts, permissions, and handoff workflows.' },
  { id: 'lease-manager', name: 'AI Lease Manager', price: 0, quote: true, startingAt: 25000, copy: 'Productized agent leasing, customer onboarding, and lease tracking.' },
  { id: 'portal', name: 'Client Portal', price: 9500, quote: false, copy: 'Private client access, magic links, billing visibility, and project handoff.' },
  { id: 'documents', name: 'Documents', price: 7500, quote: false, copy: 'Templates, sending, signatures, and stored client paperwork.' },
  { id: 'payments', name: 'Payments', price: 5000, quote: false, copy: 'Stripe payments, invoices, subscriptions, and payment history.' },
  { id: 'voice', name: 'Voice Operations', price: 12500, quote: false, copy: 'Twilio and ElevenLabs call paths, post-call summaries, and dispatch.' },
  { id: 'migrations', name: 'Migration Tools', price: 7500, quote: false, copy: 'Data import posture, JSON-to-SQLite handling, source-of-truth checks, and rollout planning.' },
  { id: 'cicd', name: 'CI/CD Discipline', price: 0, quote: true, copy: 'Build checks, deployment rules, restore points, production runbook, and controlled release flow.' },
  { id: 'white-label', name: 'White Label', price: 0, quote: true, startingAt: 25000, copy: 'Vertical workflows, branded deployment, and package-specific customization.' },
  ...ADDITIONAL_COMMAND_CENTER_MODULES.map(mod => ({
    ...mod,
    ...(mod.id === 'product-catalog' ? { copy: 'Catalog, order records, product checkout path, and follow-up operations.' } : {}),
    quote: false,
    storefronts: ['farrington-development'],
  })),
]
...
export const DEFAULT_CATALOG = {
  ...
  products: [
    ...
    createOctiCcProduct(COMMAND_CENTER_MODULES),
  ].filter(Boolean),
}
...
export const STOREFRONTS = ['farrington-development', 'octi-cc']
function stringIds(value) {
  return Array.isArray(value) ? [...new Set(value.map(v => cleanId(v)).filter(Boolean))].slice(0, 100) : []
}
function amount(value, fallback = 0) {
  const number = Number(value ?? fallback)
  return Number.isFinite(number) ? Math.max(0, number) : fallback
}
function normalizeSalesPolicy(policy = {}) {
  return {
    refunds: policy.refunds === 'window' ? 'window' : 'none',
    refundWindowDays: policy.refundWindowDays == null || policy.refundWindowDays === '' ? null : Math.floor(amount(policy.refundWindowDays)),
    statement: cleanString(policy.statement, '', 2000),
    trialUrl: cleanString(policy.trialUrl, '', 500),
    digitalConsentRequired: Boolean(policy.digitalConsentRequired),
  }
}
...
function normalizePackage(pkg = {}, index = 0) {
  ...
  return {
    ...
    storefronts: stringIds(pkg.storefronts),
    deploymentOptions: stringIds(pkg.deploymentOptions).filter(id => ['self-install', 'hosted'].includes(id)),
    hostingAddOnId: cleanId(pkg.hostingAddOnId),
    updatesIncludedMonths: Math.floor(amount(pkg.updatesIncludedMonths)),
  }
}
...
function normalizeModule(mod = {}, index = 0) {
  ...
  return {
    ...
    price: amount(mod.price),
    quote: Boolean(mod.quote),
    startingAt: mod.startingAt == null || mod.startingAt === '' ? null : amount(mod.startingAt),
    stripePriceId: cleanString(mod.stripePriceId, '', 160),
    storefronts: stringIds(mod.storefronts),
  }
}
...
function normalizeAddOn(addOn = {}, index = 0) {
  ...
  return {
    ...
    storefronts: stringIds(addOn.storefronts),
    category: ['hosting', 'service', 'module'].includes(addOn.category) ? addOn.category : 'service',
    billingInterval: ['month', 'year', 'week', 'day'].includes(addOn.billingInterval) ? addOn.billingInterval : null,
    appliesToPackages: stringIds(addOn.appliesToPackages),
  }
}
...
function normalizeSupportPlan(plan = {}, index = 0) {
  ...
  return {
    ...
    storefronts: stringIds(plan.storefronts),
    stripePriceId: cleanString(plan.stripePriceId, '', 160),
  }
}
...
export function normalizeProduct(product = {}, index = 0) {
  ...
  const paymentOptions = Array.isArray(product.paymentOptions) && (product.paymentOptions.length || id !== 'farrington-command-center')
    ? product.paymentOptions.map(normalizePaymentOption).filter(o => o.id)
    : (id === 'farrington-command-center' ? DEFAULT_PAYMENT_OPTIONS : [])
  const legacy = id === 'farrington-command-center'
  const addOns = Array.isArray(product.addOns) && (product.addOns.length || !legacy)
    ? product.addOns.map(normalizeAddOn).filter(a => a.id)
    : (legacy ? DEFAULT_ADD_ONS.map(normalizeAddOn) : [])
  const licenseTemplates = Array.isArray(product.licenseTemplates) && (product.licenseTemplates.length || !legacy)
    ? product.licenseTemplates.map(normalizeLicenseTemplate).filter(t => t.id)
    : (legacy ? DEFAULT_LICENSE_TEMPLATES : [])
  const supportPlans = Array.isArray(product.supportPlans) && (product.supportPlans.length || !legacy)
    ? product.supportPlans.map(normalizeSupportPlan).filter(p => p.id)
    : (legacy ? DEFAULT_SUPPORT_PLANS.map(normalizeSupportPlan) : [])
  const defaultPackageId = cleanId(product.defaultPackageId, packages[0]?.id || '')

  return {
    id,
    slug: cleanId(product.slug, id),
    storefronts: stringIds(product.storefronts),
    salesPolicy: normalizeSalesPolicy(product.salesPolicy || {}),
...
export function normalizeCatalog(raw = {}) {
  const products = Array.isArray(raw.products) ? raw.products : DEFAULT_CATALOG.products
  return {
    version: 1,
    updatedAt: raw.updatedAt || null,
    seededProductIds: stringIds(raw.seededProductIds),
...
function mergeDefaultProduct(product, defaultProduct) {
  if (!defaultProduct || product.id !== 'farrington-command-center') return product
...
export function getProductCatalog() {
  const saved = readData(CATALOG_FILE) || DEFAULT_CATALOG
  const defaults = normalizeCatalog(DEFAULT_CATALOG)
  // Merge raw saved fields before normalization fills absent values with 0/false/null.
  // Explicit operator values (including zero, false, null and empty arrays) still win.
  const products = Array.isArray(saved.products) ? saved.products : DEFAULT_CATALOG.products
  const catalog = normalizeCatalog({
    ...saved,
    products: products.map(product => mergeDefaultProduct(product, defaults.products.find(item => item.id === product.id))),
  })
  const seen = new Set(catalog.products.map(product => product.id))
  const missingDefaults = defaults.products.filter(product => !seen.has(product.id) && !catalog.seededProductIds.includes(product.id))
  return {
    ...catalog,
    products: [
      ...catalog.products,
...
export function saveProductCatalog(catalog) {
  ...
  normalized.seededProductIds = [...new Set([...normalized.seededProductIds, ...DEFAULT_CATALOG.products.map(p => p.id)])]
...
export function publicProduct(product) {
  return {
    ...
    packages: product.packages.map(pkg => ({
      ...
      deploymentOptions: pkg.deploymentOptions,
      hostingAddOnId: pkg.hostingAddOnId,
      updatesIncludedMonths: pkg.updatesIncludedMonths,
    })),
...
// Untagged products belong to the legacy OpenOcti storefront. Untagged
// package/module/add-on/support items inherit the product's outer visibility.
export function visibleOnStorefront(item, storefront, { product = false } = {}) {
  if (!storefront) return true
  if (item.storefronts?.length) return item.storefronts.includes(storefront)
  return !product || storefront === 'farrington-development'
}

export function filterProductForStorefront(product, storefront) {
  if (!visibleOnStorefront(product, storefront, { product: true })) return null
  if (!storefront) return product
  const filtered = { ...product }
  for (const key of ['packages', 'modules', 'addOns', 'supportPlans']) {
    filtered[key] = product[key].filter(item => visibleOnStorefront(item, storefront))
  }
  if (!filtered.packages.some(pkg => pkg.id === filtered.defaultPackageId)) filtered.defaultPackageId = filtered.packages[0]?.id || ''
  return filtered
}

export function getPublicProducts(storefront) {
  return getProductCatalog().products
    .filter(product => product.status === 'published')
    .map(product => filterProductForStorefront(product, storefront))
    .filter(Boolean)
    .map(publicProduct)
}
