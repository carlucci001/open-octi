...
export async function runSchedulerTick(options = {}) {
  ...
  try {
    ...
    try {
      const maybeRunMyvtcContactSync = options.myvtcSync
        || (process.env.NODE_ENV === 'test' ? null : (await import('./myvtc/sync')).maybeRunMyvtcContactSync)
      if (maybeRunMyvtcContactSync) await maybeRunMyvtcContactSync({ now: new Date(now) })
    } catch (error) { console.warn('[myvtc] hourly contact sync failed:', error?.message || error) }
