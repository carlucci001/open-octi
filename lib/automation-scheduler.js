...
export function isAutomationDue(automation, now = Date.now()) {
  ...
  const config = automation.trigger.config || {}
  if (config.cadence === 'daily' && Number.isInteger(config.hour)) {
    const parts = value => Object.fromEntries(new Intl.DateTimeFormat('en-CA', { timeZone: config.timeZone || 'America/New_York', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).formatToParts(new Date(value)).map(part => [part.type, part.value]))
    const today = parts(now)
    if (Number(today.hour) * 60 + Number(today.minute) < config.hour * 60 + (config.minute || 0)) return false
    if (!automation.lastRunAt || !Number.isFinite(Date.parse(automation.lastRunAt))) return true
    const last = parts(automation.lastRunAt)
    return `${today.year}-${today.month}-${today.day}` !== `${last.year}-${last.month}-${last.day}`
  }
  ...
  try {
    ...
    for (const automation of due) {
      ...
      try {
        ...
        recordScheduledRun(automation.id, { startedAt, ok: true, summary, result: execution.result })
