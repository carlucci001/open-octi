...
import { getLeadSource, LEAD_SOURCE_ROOT, loadLeadSourceRegistry, loadSourceValidations, mirrorLeadSourceStatus } from './registry'
...
export function computeScorecard({ manifest, rows, stats = {}, existingLeads = [], now = new Date() }) {
  ...
  const contactability = ratio(rows, row => Boolean(row.entity?.phone || row.entity?.email || row.entity?.website))
  const phonePresent = Number.isFinite(stats.phonePresent) ? stats.phonePresent : ratio(rows, row => Boolean(row.entity?.phone))
  ...
  const passed = rows.length > 0 && dimensions.reachability === 1 && dimensions.compliance === 1
    ...
    && dimensions.phonePresent >= (thresholds.phonePresent ?? 0)
    && (thresholds.fileAgeDays === undefined || (Number.isFinite(stats.fileAgeDays) && stats.fileAgeDays <= thresholds.fileAgeDays))
...
function writeProvingNote(validation) {
  ...
  const mapped = Object.entries(validation.fieldMapPreview.mapped).map(([target, source]) => `${target} -> ${typeof source === 'string' ? source : JSON.stringify(source)}`).join(', ')
...
export async function persistLeadSourceValidation(validation, { index = false } = {}) {
  ...
  // Portable evidence ships with the manifests; SQLite remains the live store.
  fs.writeFileSync(path.join(LEAD_SOURCE_ROOT, '_proving', validation.sourceId, 'validation.json'), JSON.stringify({ ...persisted, note: path.relative(LEAD_SOURCE_ROOT, persisted.note) }, null, 2) + '\n')
  update('sourceValidations', record.id, { note: persisted.note })
  mirrorLeadSourceStatus(validation.sourceId, validation.status)
...
export async function proveLeadSource({ sourceId, jurisdiction = {}, since, limit = 50, signalOptions = {}, pull = pullLeadSignals, persist = true, index = false, onProgress } = {}) {
  ...
  try {
    ...
    if (manifest.platform !== 'socrata') throw wrapped
    result = { rows: [], stats: { reachability: false, schemaMatched: false, error: wrapped.message, fetched: 0 } }
  }
