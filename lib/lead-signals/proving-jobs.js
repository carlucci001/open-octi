...
const BULK_FILE_JOB_TIMEOUT_MS = 30 * 60_000
...
export function runSourceProvingJob(id, options = {}) {
  ...
  const maximumTimeoutMs = job.sourceId === 'cslb-ca-contractors' ? BULK_FILE_JOB_TIMEOUT_MS : JOB_TIMEOUT_MS
  const timeoutMs = Math.min(Math.max(Number(options.timeoutMs) || maximumTimeoutMs, 1_000), maximumTimeoutMs)
  const task = (async () => {
    try {
