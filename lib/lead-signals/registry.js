...
export function loadShippedSourceValidations() {
  const dir = path.join(LEAD_SOURCE_ROOT, '_proving')
  if (!fs.existsSync(dir)) return []
  return fs.readdirSync(dir, { withFileTypes: true }).filter(entry => entry.isDirectory()).flatMap(entry => {
    try { return [JSON.parse(fs.readFileSync(path.join(dir, entry.name, 'validation.json'), 'utf8'))] } catch { return [] }
  })
}

export function loadSourceValidations() {
  return [...loadShippedSourceValidations(), ...loadAll('sourceValidations')]
}
...
export function loadLeadSourceRegistry({ syncStore = true, force = false } = {}) {
  ...
  const validations = syncStore ? loadSourceValidations() : loadShippedSourceValidations()
  const manifests = files.map(file => {
    ...
    return latest && parsed.proving.status !== 'excluded-from-build' ? { ...parsed, proving: { ...parsed.proving, status: latest.status === 'proven' ? 'proven' : 'candidate', verdict: latest.status, score: latest.score, jurisdiction: latest.jurisdiction } } : parsed
  })
...
export function mirrorLeadSourceStatus(sourceId, status) {
  if (!['proven', 'rejected'].includes(status)) throw new Error('Invalid proving verdict')
  const file = markdownFiles(LEAD_SOURCE_ROOT).find(file => parseManifestMarkdown(fs.readFileSync(file, 'utf8')).id === sourceId)
  if (!file) throw new Error(`Unknown lead source: ${sourceId}`)
  const markdown = fs.readFileSync(file, 'utf8')
  const manifest = parseManifestMarkdown(markdown)
  if (manifest.proving.status === 'excluded-from-build') return
  const proving = { ...manifest.proving, status }
  fs.writeFileSync(file, markdown.replace(/^proving:[^\r\n]*/m, `proving: ${JSON.stringify(proving)}`))
  cache.signature = ''
}
