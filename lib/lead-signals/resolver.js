...
import { loadLeadSourceRegistry, loadSourceValidations } from './registry'
...
export function jurisdictionForLocation(location, supplied = {}) {
  const region = String(location || '').trim().toUpperCase()
  if (['US', 'UNITED STATES'].includes(region)) return { country: 'US' }
  if (/^[A-Z]{2}$/.test(region)) return { ...supplied, state: region }
...
export function jurisdictionMatches(source, jurisdiction) {
  if (jurisdiction.country === 'US') return source.coverage?.some(value => /^[A-Z]{2}(?:-|$)/.test(value))
...
function provenFor(source, jurisdiction, validations) {
  ...
  const relevant = validations.filter(validation => {
    if (validation.sourceId !== source.id) return false
    const target = validation.jurisdiction || {}
    if (jurisdiction.country === 'US') return Boolean(target.state && jurisdictionMatches(source, target))
  ...
  return relevant[0]?.status === 'proven'
}
...
export function resolveLeadSources({ leadType, location, jurisdiction: supplied, manifests, validations, includeCandidates = false } = {}) {
  ...
  const history = validations || loadSourceValidations()
  const jurisdiction = jurisdictionForLocation(location, supplied)
  const triggers = triggersForLeadType(leadType, registry)
  const ranked = registry.filter(source => (source.tier === 'A' || source.platform === 'bulk-file')
    && (slug(leadType) !== 'new-businesses' || source.platform === 'socrata')
    && jurisdictionMatches(source, jurisdiction)
    && source.triggers?.some(trigger => triggers.includes(trigger))).map(source => {
