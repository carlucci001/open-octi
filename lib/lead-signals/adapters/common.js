...
export function getPath(value, pathSpec) {
  if (pathSpec && typeof pathSpec === 'object' && !Array.isArray(pathSpec)) {
    if (pathSpec.join) return pathSpec.join.map(spec => getPath(value, spec)).filter(item => item != null && String(item).trim()).join(pathSpec.separator ?? ' ')
    if ('literal' in pathSpec) return pathSpec.literal
  }
...
export function signalSinceDate(value, { firstRun = false, backfill = false, now = new Date() } = {}) {
  const relative = String(value || '').match(/^-(\d+)d$/)
  const date = relative ? new Date(now.getTime() - Number(relative[1]) * 86400000)
    : value ? new Date(value) : new Date(now.getTime() - (firstRun || backfill ? 14 : 3) * 86400000)
  if (Number.isNaN(date.getTime())) throw new Error('Invalid signal since date')
  return date.toISOString().slice(0, 10)
}

export function requestContext({ jurisdiction = {}, since, limit = 50, offset = 0, query = '' } = {}, apiKey = '') {
  const today = new Date().toISOString().slice(0, 10)
  const sinceDate = signalSinceDate(since || '-30d')
  return {
...
export function normalizeSignal(raw, manifest) {
  ...
  return {
    ...
    people: fields.people ? fields.people.map(person => ({ name: clean(getPath(raw, person.name)), title: person.title })).filter(person => person.name)
      : personName ? [{ name: personName, title: credential || 'Provider' }] : [],
