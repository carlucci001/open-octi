...
export async function openclawChat({ message, sessionKey = DEFAULT_SESSION, token, onChunk, firstChunkMs = 45000, betweenChunksMs = 6000, maxMs = 300000 }) {
  return new Promise((resolve, reject) => {
    ...
    let runId = null
    const earlyEvents = []
    let hasAssistantSnapshots = false
    ...
    ws.on('open', async () => {
      try {
        ...
        await call('connect', {
          ...
          client: { id: 'openclaw-control-ui', version: 'openocti', platform: 'web', mode: 'webchat', instanceId: randomUUID() },
          role: 'operator',
        ...
        runId = r?.runId
        if (!runId) throw new Error('OpenClaw did not acknowledge this conversation run')
        for (const event of earlyEvents.splice(0)) handleEvent(event)
        if (!settled) armIdle() // start the "waiting for first chunk" timer
      } catch (e) { done(e) }
    })
    ...
    const handleEvent = (m) => {
      ...
      // Gateway events are broadcasts. No content is accepted without an exact run match.
      ...
      if (m.event === 'chat') {
        ...
        if (state === 'delta') {
          // The gateway can send the same text on both agent and chat streams.
          if (!hasAssistantSnapshots) fullText = mergeOpenClawChatDelta(fullText, payload)
          gotFirstChunk = true
          if (typeof onChunk === 'function') try { onChunk(fullText) } catch {}
          armIdle()
        }
    ...
    ws.on('message', (data) => {
      ...
      if (m.type !== 'event' || !['agent', 'chat'].includes(m.event)) return
      if (!runId) {
        // Preserve an early event from our own run, but never render it before the ack.
        if (m.payload?.runId && earlyEvents.length < 256) earlyEvents.push(m)
        return
      }
      handleEvent(m)
    })
